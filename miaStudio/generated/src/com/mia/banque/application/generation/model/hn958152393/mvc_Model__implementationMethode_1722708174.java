...
public class mvc_Model__implementationMethode_1722708174 implements com.mia.studio.kernel.compiler.CompiledAction {

	public mvc_Model__implementationMethode_1722708174() {
		//Do nothing
	}

	//For internal use only
	mvc_Model__implementationMethode_1722708174(int anyInt, List anyList) {
			...
			try {			
				...
				throw new IllegalArgumentException(
						String.format(
								...
								"implementationMethode", argument.getClass().getName(),
	...
	public String runAction(java.lang.Object[] _action_parameters) {
		if (_action_parameters.length != 3)
			throw new IllegalArgumentException("Wrong number of parameters in call to ''implementationMethode''");
...
for (Model element : current.getOwnedElements()){
	result += manager.getString(element, "variableGetterImplementation");
	result += manager.getString(element, "variableSetterImplementation");
}

for (ModelProperty property : current.getProperties()){
	result += manager.getString(property, "variableGetterImplementation");
	result += manager.getString(property, "variableSetterImplementation");
}
