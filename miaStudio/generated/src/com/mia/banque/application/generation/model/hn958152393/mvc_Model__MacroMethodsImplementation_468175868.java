...
public class mvc_Model__MacroMethodsImplementation_468175868 implements com.mia.studio.kernel.compiler.CompiledAction {

	public mvc_Model__MacroMethodsImplementation_468175868() {
		//Do nothing
	}

	//For internal use only
	mvc_Model__MacroMethodsImplementation_468175868(int anyInt, List anyList) {
			...
			try {			
				...
				throw new IllegalArgumentException(
						String.format(
								...
								"MacroMethodsImplementation", argument.getClass().getName(),
	...
	public String runAction(java.lang.Object[] _action_parameters) {
		if (_action_parameters.length != 3)
			throw new IllegalArgumentException("Wrong number of parameters in call to ''MacroMethodsImplementation''");
...
for (Model element : current.getOwnedElements()){
	result += manager.getString(element, "TextTemplateVariableGetterImplementation");
	result += manager.getString(element, "TextTemplateVariableSetterImplementation");
}

for (ModelProperty property : current.getProperties()){
	result += manager.getString(property, "TextTemplateVariableGetterImplementation");
	result += manager.getString(property, "TextTemplateVariableSetterImplementation");
}
