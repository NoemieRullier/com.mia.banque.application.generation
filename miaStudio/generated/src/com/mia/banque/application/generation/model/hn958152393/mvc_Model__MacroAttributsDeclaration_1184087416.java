...
public class mvc_Model__MacroAttributsDeclaration_1184087416 implements com.mia.studio.kernel.compiler.CompiledAction {

	public mvc_Model__MacroAttributsDeclaration_1184087416() {
		//Do nothing
	}

	//For internal use only
	mvc_Model__MacroAttributsDeclaration_1184087416(int anyInt, List anyList) {
			...
			try {			
				...
				throw new IllegalArgumentException(
						String.format(
								...
								"MacroAttributsDeclaration", argument.getClass().getName(),
	...
	public String runAction(java.lang.Object[] _action_parameters) {
		if (_action_parameters.length != 3)
			throw new IllegalArgumentException("Wrong number of parameters in call to ''MacroAttributsDeclaration''");
...
for (Model element : current.getOwnedElements()){
	 result += manager.getString(element, "variableDeclaration");
}

for (ModelProperty property : current.getProperties()){
	result += manager.getString(property, "variableDeclaration");
}
